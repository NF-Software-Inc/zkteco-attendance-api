...
namespace zkteco_attendance_api
{
	...
	/// </summary>
	internal abstract class CommandBase<TPacket> where TPacket : class, IZkPacket, new()
	{
		...
		public virtual IZkPacket? SendCommand(Commands command, string? data, int length)
		{
			...
			if (packet.Data.Length % 2 != 0)
				packet.Data = packet.Data.Concat(new byte[] { 0x00 }).ToArray();
		...
		protected virtual IZkPacket? GetBufferData(byte[] data)
		{
			...
			var packet = SendCommand(Commands.ClearBuffers, Array.Empty<byte>(), ZkPacketBase.DefaultHeaderLength);
