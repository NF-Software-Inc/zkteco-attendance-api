...
namespace zkteco_attendance_api
{
	internal static class Functions
	{
		public static byte[] GeneratePassword(int password, int session)
		{
			...
			bytes = BitConverter
				.GetBytes(BitConverter.ToUInt16(new byte[] { (byte)(bytes[2] ^ 'S'), (byte)(bytes[3] ^ 'O') }))
				.Concat(BitConverter.GetBytes(BitConverter.ToUInt16(new byte[] { (byte)(bytes[0] ^ 'Z'), (byte)(bytes[1] ^ 'K') })))
				.ToArray();
